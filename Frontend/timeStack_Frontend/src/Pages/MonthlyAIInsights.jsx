...
export  function MonthlyAIInsights() {
  ...
  const handleGetAnalysis = async () => {
    try {
      ...
      const response = await api.get("task/stats/monthlyTaskInsights");

      setSummary(response.data.summary);
      setInsight(response.data.insight);
    } catch (err) {
      console.error("Error fetching monthly AI insights:", err);

      setError(
        err.response?.status === 503
          ? "AI service is temporarily unavailable. Please try again later."
          : "Failed to fetch monthly AI insights."
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="w-full min-h-screen bg-white pt-24 px-4 sm:px-8 lg:px-10 flex justify-center">
      ...
      >
        ...
        <p className="text-gray-700 text-base sm:text-lg leading-relaxed mb-8">
          ...
          your time this month. Understand what took the most time, what was
          underutilized, and how you can improve next month.
        </p>

        <div className="mb-8">
          <RaisedButton
            text={loading ? "Loading..." : "Get monthly analysis"}
